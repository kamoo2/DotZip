      ...
      <b-card class="mb-2">
        <b-form>
          ...
          <b-row class="button-wrap">
            ...
            <div v-if="flag">
              <b-button @click="onClickDeleteBookmarkedArea" v-if="isBookmarkedArea" class="mb-1 mr-4" variant="danger"
                >관심 지역 해제</b-button
              >
              <b-button @click="onClickAddBookmarkedArea" v-else class="mb-1 mr-4" variant="success"
                >관심 지역 등록</b-button
              >
...
export default {
  ...
  computed: {
    ...mapGetters(["sidoList", "gugunList", "dongList", "houseList", "house", "currentUser", "isBookmarkedArea"]),
  },
  watch: {
    houseList() {
    ...
    dong() {
      if (this.dong !== null && this.dong.value !== null) {
        ...
        this.getBookMarkAreaAction({dongCode, userSeq: this.currentUser.userSeq});
      }
    },
  },
  methods: {
    ...
    onClickDeleteBookmarkedArea() {
      this.deleteBookMarkAreaAction({userSeq: this.currentUser.userSeq, dongCode: this.dong.value});
      this.$store.commit("SET_IS_BOOKMARK_AREA", false);
      this.$notify("error", "북마크 삭제", "해당 지역의 북마크가 삭제되었습니다.", {
        duration: 3000,
        permanent: false,
      });
    },
    onClickAddBookmarkedArea() {
      this.addBookMarkAreaAction({userSeq: this.currentUser.userSeq, dongCode: this.dong.value});
      this.$store.commit("SET_IS_BOOKMARK_AREA", true);
      this.$notify("success", "북마크 등록", "해당 지역의 북마크가 등록되었습니다.", {
        duration: 3000,
        permanent: false,
      });
    },
    ...
    async onClickSearchBtn() {
      ...
      if (this.sido.value === null) {
        ...
        await this.getHouseList({dong: this.dong.label, userSeq: this.currentUser.userSeq});
