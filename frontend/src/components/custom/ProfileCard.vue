<template>
  <b-card no-body>
    <b-card-body
      class="pt-5 pb-5 d-flex flex-lg-column flex-md-row flex-sm-row flex-column"
    >
      <div class="price-top-part">
        <h5 class=" mb-0 font-weight-semibold color-theme-1 mb-4">
          {{ data.name }}
        </h5>
        <p class="text-large mb-2 text-default"></p>
        <p class="text-muted text-small">{{ data.info }}</p>
      </div>
      <!-- <div
      ...
      >
        <ul class="list-unstyled">
          <li
            v-for="(feature, fIndex) in data.features"
            :key="`feature_${fIndex}`"
          >
